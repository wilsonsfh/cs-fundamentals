...
|---------|-----------|-------------|------|
| Reverse Words in a String | Medium | split() + reverse + join | [LC 151](https://leetcode.com/problems/reverse-words-in-a-string/) |
| String to Integer (atoi) | Medium | strip, sign, digit-by-digit | [LC 8](https://leetcode.com/problems/string-to-integer-atoi/) |
| Valid Anagram | Easy | split into chars, sort or count | [LC 242](https://leetcode.com/problems/valid-anagram/) |
| Decode String | Medium | stack-based parsing | [LC 394](https://leetcode.com/problems/decode-string/) |

## Flashcards

Strip last N chars from a string?::`s[:-N]` e.g. `"29th"[:-2]` → `"29"`

Zero-pad a number to width 2?::`f"{n:02d}"` e.g. `4` → `"04"`

Split `"29th Oct 2052"` into parts?::`s.split()` → `["29th", "Oct", "2052"]`

Get month number without a dict?::`months.index("Oct") + 1` where months is an ordered list

Join a list into a string?::`"-".join(["2052", "10", "29"])` → `"2052-10-29"`

When to use `split()` vs `split(" ")`?::`split()` handles multiple spaces and strips edges; `split(" ")` is literal and can produce empty strings
